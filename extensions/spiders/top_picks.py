...
class TopPicksSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        base_url = r'https://chrome.google.com/webstore/ajax/item?' \
                   ...
                   r'%2Ftop_picks_:change1:&sortBy=0&container=CHROME&_reqid=:change2:&rt=j'
        ...
        for cat_iter in re.finditer(cat_line_regex, all_content):
            ...
            bygoogle_regex = re.compile(r'by-google')
            if category_code_id and re.search(bygoogle_regex, category_code_id) == None:
                code_id = re.search(r'\d+-(.+)$', category_code_id).group(1)
                url = re.sub(r':change1:', code_id, base_url)
